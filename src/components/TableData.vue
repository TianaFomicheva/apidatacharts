    ...
    <template v-slot:default>
      ...
      <tbody>
        <tr
          ...
          :key="item.name"
        >
          <td><div style="height:12px;width:12px;display:inline-block; margin-right:4px;" :style="{background:item.color}"></div>{{ item.name }}</td>
  ...
  mounted(){
    this.sum = this.percents.reduce((a, b) => a + b, 0)
      for(let i=0;i<this.labels.length;i++){
          ...
          this.groupsList[i]['percent'] = this.percents[i] !==0 ?  this.sum/this.percents[i]*100 : 0
